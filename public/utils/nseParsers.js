...
function parseNseDateFromFilename(fileName) {
    ...
    // Use word boundaries or start/end of string to be safe
    const ddmmyyyyMatchFile = fileName.match(/(?:^|_)(\d{2})(\d{2})(\d{4})(?:$|\.)/);
    ...
    const yyyymmddMatch = fileName.match(/(?:^|_)(\d{4})(\d{2})(\d{2})(?:$|\.)/);
    if (yyyymmddMatch) {
    ...
    // Be very strict here to avoid matching random numbers
