...
async function generateSimpleMomentumGapSignals(date, strategy = 'momentum_gap') {
  try {
    ...
    console.log(`   - Checking for EOD data: ${date} (today) OR ${yesterdayDate} (yesterday)`);
    console.log(`   - Premarket data: ${date} (today's pre-open)`);

    // VALIDATION: Check if we have necessary data BEFORE proceeding
    // 1. Check for bhavcopy - try TODAY first (if market closed), then YESTERDAY
    ...
    // Get bhavcopy data - TRY TODAY FIRST (for after market close), then YESTERDAY
    let bhavcopyData = [];
    let bhavCountDaily = 0;
    let bhavCountUploaded = 0;
    let actualBhavDate = null;
    const MIN_BHAV_COUNT = 300; // Minimum valid bhav records (NSE typically has 300+ EQ stocks)

    try {
      // PRIORITY 1: Try TODAY's date first (for EOD mode after market close)
      console.log(`🔍 Checking for EOD data on TODAY (${date})...`);
      let todayBhavData = await bhavcopyCollection
        .find({
          date: date,
          series: 'EQ'
        })
      ...
      const todayBhavCount = todayBhavData.length;
      console.log(`📊 TODAY (${date}) bhav count: ${todayBhavCount}`);

      if (todayBhavCount >= MIN_BHAV_COUNT) {
        console.log(`✅ Using TODAY's EOD data: ${date} with ${todayBhavCount} records`);
        bhavcopyData = todayBhavData;
        bhavCountDaily = todayBhavCount;
        actualBhavDate = date;
      } else {
        // PRIORITY 2: Fall back to YESTERDAY's date (for premarket mode)
        console.log(`⚠️ TODAY's EOD insufficient (${todayBhavCount} records). Trying YESTERDAY (${yesterdayDate})...`);
        ...
        console.log(`📊 YESTERDAY (${yesterdayDate}) bhav count: ${bhavCountDaily}`);

        if (bhavCountDaily >= MIN_BHAV_COUNT) {
          console.log(`✅ Using YESTERDAY's EOD data: ${yesterdayDate} with ${bhavCountDaily} records`);
          actualBhavDate = yesterdayDate;
        }
      }
      ...
      if (bhavcopyData.length < MIN_BHAV_COUNT) {
        if (bhavcopyData.length > 0) {
          console.warn(`⚠️ Bhav data has only ${bhavcopyData.length} records (below minimum ${MIN_BHAV_COUNT}). Searching for valid alternative...`);
        } else {
          console.log(`No bhav data for ${date} or ${yesterdayDate}, searching for closest valid date...`);
        }
